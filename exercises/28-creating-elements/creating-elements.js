(function () {
  ...
  const handleSubmit = (e) => {
    ...
    inputParagraph.classList.add("italic");
    inputParagraph.textContent = `No results for ${ocean} found`;
    document.querySelector("#inputResult").appendChild(inputParagraph);
  };
  const handleThisForm = document.querySelector("#handleThisForm");

  handleThisForm.addEventListener("submit", handleSubmit);
  ...
  // Write you JavaScript here
  let chatGPTConversation = document.querySelector("#chatGPTConversation");
  ...
  for (const dialogue of conversationDialogues) {
    ...
    chatGPTConversation.insertAdjacentHTML("beforeend", htmlConversation);
  }
})();
